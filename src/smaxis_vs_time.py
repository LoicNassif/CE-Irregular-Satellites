...
def mass_comp(s, t):
    Qd = s.computeQd(s.Dc)
    part1 = 1.3e7 * (s.M_s/1.989e30)**1.38 * s.f_vrel**2.27
    part2 = (Qd**0.63 * s.rho * (s.Dmax/1000) * (s.M_pl/5.972e24)**0.24
                * s.eta**4.13 * (s.a_pl/1.496e11)**4.13)
    return ((1/t)  * part2 / part1)


#main
def main(swarm_argv, lamb, t, M0flag=False):
    ...
    if not M0flag:
        ...
        for i in range(len(t)):
            ...
            #T = s.computeT(L_s, a_plv)
            #bnu = s.computeBmu(lamb, T)
            s2 = swarms.CollSwarm(M0v*5.972e24, Dt, Dmax, L_s, M_s, M_pl,
                                    a_pl, R_pl, eta, Nstr, d_pl,
                                    rho=RHO, fQ=5, f_vrel=4/pi,
                                    correction=True, alpha=1.2)
            F_th = s2.computeFth(array([lamb]), swarm=True)
    ...
    if not M0flag:
        ...
        plt.figure(1)
        plt.plot(M0v_list, fth)
        plt.ticklabel_format(style='sci', axis='both', scilimits=(0,0))
        plt.xlabel("initial Mass [Earth Mass]")
        plt.ylabel("F_th [Jy]")
        plt.loglog()
        plt.show()
    plt.figure(2)
